...
export function HPSTimelineChart() {
  const { selectedSession, sessionSummary } = useSession();

  const chartData = useMemo(() => {
    if (!selectedSession || !sessionSummary) return [];

    const hpsCalculator = new DPSCalculator(5000); // 5s window
    const { startTime, events } = selectedSession;

    // Get top 5 healers
    const topHealers = sessionSummary.healingMeter
      .slice(0, 5)
      .map((e) => e.entity.name);

    if (topHealers.length === 0) return [];

    // Calculate HPS timeline for each entity
    const entityTimelines: Record<string, Map<number, number>> = {};

    for (const entityName of topHealers) {
      const entityEvents = events.filter(
        (e): e is HealingEvent =>
          e.eventType === EventType.HEALING_DONE && e.source.name === entityName
      );

      if (entityEvents.length > 0) {
        const timeline = hpsCalculator.calculateHPSTimeline(entityEvents, 1000);
  ...
  const entityNames = useMemo(() => {
    ...
    return sessionSummary.healingMeter.slice(0, 5).map((e) => e.entity.name);
  }, [sessionSummary]);

  if (chartData.length === 0) {
    return (
      <WidgetContainer title="HPS Over Time" size="large">
